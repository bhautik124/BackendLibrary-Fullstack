...
const Create = () => {
  ...
  const handleCreateCollection = async () => {
    ...
    // Validate fields: ensure all field names are provided
    const invalidFields = fields.some((field) => !field.name.trim());
    if (invalidFields) {
    ...
    if (isAuth === null) {
      setCollectionError(
        "Authentication check is still loading. Please try again."
      );
      return;
    }
  ...
  "https://backendlibraryy-fullstack-backend.onrender.com/api/create-collection-name",
        { collectionName, fields },
        { withCredentials: true }
      );

      setCollectionSuccess(response.data.message);

      // Check if fields were added to existing model
      if (response.data.addedFields && response.data.allFields) {
        setAddedFields(response.data.addedFields);
        setAllFields(response.data.allFields);
        setIsFieldsAdded(true);
      }

      setFields([{ name: "", type: "String" }]); // Reset fields after success
    } catch (err) {
      console.error(err);
      const errorMessage =
      ...
      <div className="mt-5">
        ...
        <div className="ml-2 sm:ml-10">
          <h3 className="text-md sm:text-md font-semibold mb-2">
            This API allows you to create resources such as a{" "}
            ...
            , or any other collection you need by defining a model and its
            fields.
          </h3>
        </div>
      </div>
        ...
        <div className="mb-3 mt-5 sm:ml-10">
          <div className="flex items-start gap-3 mb-4">
            ...
            <p className="text-base sm:text-lg">
              ...
              , and{" "}
              <code className="bg-stone-800 text-zinc-200 px-2 py-1 rounded text-xs sm:text-sm font-mono">
                add data to the selected model
              </code>
              .
            </p>
          </div>
        ...
        <div className="mb-3 mt-5 sm:ml-10">
          <div className="flex items-start gap-3 mb-4">
            ...
            <p className="text-base sm:text-lg">
              ...
              field types are supported. Support for{" "}
              <code className="bg-stone-800 text-zinc-200 px-2 py-1 rounded text-xs sm:text-sm font-mono">
                array
              </code>{" "}
              and{" "}
            ...
            {fields.map((field, index) => (
              ...
              >
                ...
                <button
                  ...
                  className="text-2xl sm:text-4xl text-red-500 hover:text-red-700 transition-colors"
                >
                  🗑
                </button>
              </div>
