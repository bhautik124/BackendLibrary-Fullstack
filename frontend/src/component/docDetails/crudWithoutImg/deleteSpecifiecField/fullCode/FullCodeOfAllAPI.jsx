...
const Home = () => {
  ...
  const handleDelete = async (collectionName, documentId) => {
    const confirmDelete = window.confirm(
      \`Are you sure you want to delete document \${documentId} from \${collectionName}?\`
    );
    if (!confirmDelete) return;

    try {
      const res = await axios.delete(DELETE_API_URL, {
        headers: {
      ...
      setTimeout(() => setDeleteResponse(null), 2000);

      fetchAllModels(); // Refresh data after deletion
    } catch (err) {
      setDeleteResponse({
        error:
                ...
                <div className="space-y-3">
                  {model.data.map((item, index) => (
                    <div key={index} className="border-t border-gray-200 pt-3">
                      ...
                      <button
                        onClick={() => handleDelete(model.modelName, item._id)}
