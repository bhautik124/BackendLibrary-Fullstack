...
import GetCreatedModel2 from "./GetCreatedModel2";
import useApiGenerateCheckAuth from "../../../userLoginPage/protectedRoute/ForApiGenereateCheckAuth";
import { Navigate } from "react-router-dom";
import { useDispatch, useSelector } from "react-redux";
import { fetchDataWithImg } from "../../../../store/action/FetchDataWithImg";

const WithImgGetCreatedModel1 = () => {
  ...
  const { url, apiKey, token } = useSelector((state) => state.withImg);
  ...
  const generateAllData = async () => {
    ...
    try {
      const response = await dispatch(fetchDataWithImg());
      if (
        !response.api?.[0]?.url ||
  ...
  return (
    <div className="pb-40 px-4 sm:px-6 lg:px-8">
      <h1 className="text-zinc-100 text-2xl sm:text-4xl font-bold">
        Retrieve All Models with Their Data
      </h1>
      ...
      <div className="mt-5">
        ...
        <div className="ml-2 sm:ml-10">
          <h3 className="text-md sm:text-md font-semibold mb-2">
            This API enables the retrieval of all created models, such as{" "}
            ...
            , along with their associated data and image support.
          </h3>
        </div>
      </div>
      ...
      <div className="mt-8">
        ...
        <div className="mb-3 mt-5 sm:ml-10">
          <div className="flex items-start gap-3 mb-4">
            ...
            <p className="text-base sm:text-lg">
              This API fetches all collections and their data using the{" "}
              <code className="bg-stone-800 text-zinc-200 px-2 py-1 rounded text-xs sm:text-sm font-mono">
                CRUD Get All Models with Images
              </code>{" "}
              feature.
            </p>
          </div>
        ...
        <div className="mb-3 mt-5 sm:ml-10">
          <div className="flex items-start gap-3 mb-4">
            ...
            <p className="text-base sm:text-lg">
              Generate a token and API key for authentication to retrieve the
              list of models.
            </p>
          </div>
        </div>
      ...
      <div className="mb-3 mt-8">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-zinc-100 text-xl sm:text-2xl font-semibold">
            STEP 1 – Generate API, Token, and API Key
          </h2>
        </div>
      </div>
...
export default WithImgGetCreatedModel1;
