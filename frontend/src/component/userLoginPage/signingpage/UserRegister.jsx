...
//             <Link to="/user-login" className="text-white underline">
//               Login
//             </Link>
//           </p>
//         </div>
...
const UserRegister = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post(
        "https://backendlibrary-fullstack-backend.onrender.com/api/user/auth/register",
        { userName, email, password },
        { withCredentials: true }
      );

      toast(<CustomToast text="Registration Successful" type="success" />, {
        autoClose: 2000,
        closeButton: false,
        hideProgressBar: true,
        style: {
      ...
      if (err.response && err.response.status === 400) {
        toast(<CustomToast text={err.response.data} type="error" />, {
          autoClose: 2000,
          closeButton: false,
          hideProgressBar: true,
          style: {
